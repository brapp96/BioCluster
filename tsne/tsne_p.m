function ydata = tsne_p(P, labels, no_dims, thisaxis)
...
%         end
        ...
        if ~rem(iter, 10) && ~isempty(labels)
            if no_dims == 1
                scatter(thisaxis,ydata, ydata, 9, labels, 'filled');
            elseif no_dims == 2
                scatter(thisaxis,ydata(:,1), ydata(:,2), 30, labels, 'filled');
            else
                scatter3(thisaxis,ydata(:,1), ydata(:,2), ydata(:,3), 40, labels, 'filled');
            end
            thisaxis.XTick = [];
            thisaxis.YTick = [];
            thisaxis.XLimMode = 'auto';
            thisaxis.YLimMode = 'auto';
            thisaxis.ZLimMode = 'auto';
            drawnow
        end
    end
